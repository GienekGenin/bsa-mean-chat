 ...
 *
 * @returns {string} response with written phrase about weather,
 * if req unknown responses with random phrase.
 */
    ...
    if (days[i] === 'today' || days[i] === 'tomorrow') {
      ...
      if (rule.test(msg)) {
        for (let c = 0; c < cities.length; c++) {
          ...
          if (rule.test(msg)) {
            return `The weather will be hot AF on ${days[i]} in ${cities[c]}`;
          }
        }
      }
    }
